...
public interface EntryDyzurDbRepo extends CrudRepository<EntryDyzurDb, Long>, JpaRepositoryImplementation<EntryDyzurDb, Long> {
     ...
     */
    @Query(value = "SELECT e FROM EntryDyzurDb e WHERE e.startTime>=:previousMonth AND e.startTime<=:nextMonth")
    ...
    @Query(value = "SELECT e FROM EntryDyzurDb  e WHERE \n" +
            ...
            "AND e.hospitalDepartment.id=:hospitalDeptId ")
...
//            "AND hospital_hospital_id=:hospitalId ",nativeQuery = true)
    ...
    @Query("SELECT e.id FROM EntryDyzurDb e WHERE e.description=?1 AND e.startTime=?2 AND e.endTime=?3 " +
            "AND e.hospital=?4 AND e.title=?5")
...
//            "AND hospital_department_hospital_dept_id=:hospitalDepartment",nativeQuery = true)
    @Query(value = "SELECT e FROM EntryDyzurDb e WHERE e.startTime=:startTime \n" +
            ...
            "AND e.hospitalDepartment=:hospitalDepartment")
